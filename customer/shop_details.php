...
if (empty($error)) {
  ...
  // Fetch shop with rating stats; only show approved shops publicly and with active subscription
  $sql = "SELECT $selectCols,
            ...
            FROM Barbershops s
            WHERE s.shop_id = :id AND s.status = 'approved' AND EXISTS (SELECT 1 FROM Shop_Subscriptions subs WHERE subs.shop_id=s.shop_id AND subs.payment_status='paid' AND CURDATE() BETWEEN subs.valid_from AND subs.valid_to)";
