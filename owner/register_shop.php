...
function in_post($k, $d = '') { return isset($_POST[$k]) ? trim((string)$_POST[$k]) : $d; }
...
$batangasCities = [
	'Batangas City','Lipa','Tanauan','Sto. Tomas','Malvar','Balete','Agoncillo','Alitagtag','Balayan','Cuenca','Ibaan','Laurel','Lemery','Lian','Mabini','Nasugbu','Padre Garcia','Rosario','San Jose','San Juan','San Luis','San Nicolas','San Pascual','Sta. Teresita','Taal','Taysan','Tingloy','Calaca','Calatagan'
];
...
<head>
	...
	<style>
		.card { background: linear-gradient(135deg,#0b1624,#0a1120); border:1px solid #1d3557; border-radius: 14px; }
		.form-row { display:grid; gap:.75rem; grid-template-columns: repeat(auto-fit, minmax(220px,1fr)); }
		.small-note { font-size:.65rem; color:#93c5fd; margin-top:.25rem; }
		.alert { background:#7f1d1d; color:#fecaca; border:1px solid #991b1b; padding:.6rem .8rem; border-radius:8px; font-size:.7rem; }
	</style>
	<script>
		function confirmSubmit(e){
			if(!confirm('Create this shop now?')){ e.preventDefault(); return false; }
			return true;
		}
	</script>
