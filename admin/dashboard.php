...
<script>
  ...
  (() => {
    ...
    const monthsAll = revTrend.map(r => {
      const [y, m] = r.ym.split('-');
      return new Date(y, m - 1, 1).toLocaleString('en-US', {
        month: 'short'
      });
    });
    const seriesRevAll = revTrend.map(r => r.value);
    const seriesProfitAll = revTrend.map(r => Math.round(r.value * 0.62));

    // Helper to get slice size based on range; monthly approximation
    const getSliceSize = (range) => {
      if (range === '30d') return 1; // ~1 month
      if (range === '90d') return 3; // ~3 months
      return 12; // 1y
    };
    ...
    const sliceFor = (range) => {
      ...
      return {
        labels: monthsAll.slice(-n),
        rev: seriesRevAll.slice(-n),
        profit: seriesProfitAll.slice(-n)
      };
    };

    const initial = sliceFor(initRange);
    ...
    // Range buttons behavior
    const group = document.getElementById('rev-range-group');
    if (group) {
      group.addEventListener('click', (e) => {
        ...
        const next = sliceFor(range);
        revenueChart.data.labels = next.labels;
        revenueChart.data.datasets[0].data = next.rev;
        revenueChart.data.datasets[1].data = next.profit;
        revenueChart.update();
      });
    }
  })();
